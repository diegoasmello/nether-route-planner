...
export const COLORS = {
  background: "#0b0910",
  gridMinor: "rgba(168, 146, 196, 0.07)",
  gridMajor: "rgba(168, 146, 196, 0.16)",
  gridLabel: "rgba(168, 146, 196, 0.55)",
  axisLine: "#3a4150",
  axisText: "#8b93a3",
  hubStroke: "#f59e0b",
...
function panViewportByScreenDelta(
  viewport: Viewport,
  dxPx: number,
  dyPx: number,
): Viewport {
...
export const RouteCanvas = forwardRef<RouteCanvasHandle, RouteCanvasProps>(
  function RouteCanvas(
    ...
    useImperativeHandle(
      ...
      () => ({
        zoomIn: () =>
          setViewport((v) => ({ ...v, scale: clampScale(v.scale * 1.4) })),
        zoomOut: () =>
          setViewport((v) => ({ ...v, scale: clampScale(v.scale / 1.4) })),
        centerRoute,
      }),
    ...
    useEffect(() => {
      ...
      const handleWheel = (e: WheelEvent) => {
        ...
        const pointerScreen: ScreenPoint = {
          x: e.clientX - rect.left,
          y: e.clientY - rect.top,
        };

        setViewport((oldViewport) => {
          const worldUnderCursor = screenToWorld(
            pointerScreen,
            oldViewport,
            size.width,
            size.height,
          ...
          const guess: Viewport = {
            center: oldViewport.center,
            scale: newScale,
          };
          const screenAtGuess = worldToScreen(
            worldUnderCursor,
            guess,
            size.width,
            size.height,
          );
          return panViewportByScreenDelta(
            guess,
            pointerScreen.x - screenAtGuess.x,
            pointerScreen.y - screenAtGuess.y,
          );
        });
      };
    ...
    useEffect(() => {
      ...
      const w2s = (p: { x: number; z: number }) =>
        worldToScreen(p, viewport, size.width, size.height);

      ctx.fillStyle = COLORS.background;
      ctx.fillRect(0, 0, size.width, size.height);

      const topLeftWorld = screenToWorld(
        { x: 0, y: 0 },
        viewport,
        size.width,
        size.height,
      ...
      if (hubRadius > 0) {
        ...
        if (portalCount && portalWidth) {
          ...
          for (const slot of distributePortalsOnHub(
            hubOrigin,
            hubRadius,
            portalCount,
            portalWidth,
      ...
      const drawRoute = (route: NamedRoute, isPrimary: boolean) => {
        ...
        const corridorFill = isPrimary
          ? COLORS.corridorFill
          : COLORS.otherCorridorFill;
        const centerlineStroke = isPrimary
          ? COLORS.centerlineStroke
          : COLORS.otherCenterlineStroke;
        const accent = isPrimary ? COLORS.accent : COLORS.otherAccent;
        const destinationColor = isPrimary
          ? COLORS.destination
        ...
        if (
          route.title.trim() !== "" &&
          routeResult.tunnel.centerline.length > 0
        ) {
          const midBlock =
            routeResult.tunnel.centerline[
              Math.floor(routeResult.tunnel.centerline.length / 2)
            ];
        ...
        // Hub exit marker.
        if (
          routeResult.hub.radius > 0 &&
          !routeResult.hub.withinHub &&
          !routeResult.isSamePoint
        ) {
